...
# Load species list and ask for .bbx file
SPECIES = {}
...
for image in DATA['images']:
    ...
    for a in annotations:
        if a['label'].lower() not in SPECIES:
            print("Species list does not contain [{}]".format(a['label']))
            sys.exit(0)
print()
...
visit_id_list = []
results = cur.execute('select VisitID, LocationID from Visits where VisitTypeID = 2 order by VisitDate asc').fetchall()
for rec in results:
    visit_id_list.append(str(rec[0]))
    print('{}  {} - {}'.format(rec[0], study_area[locations[rec[1]][0]], locations[rec[1]][1]))
